...
/* [[pr_10_3 - bond constraints, angle distributions]] */
...
real *histBondAng, *histDihedAng;
int sizeHistBondAng, sizeHistDihedAng, stepChainProps;
...
NameList nameList[] = {
  ...
  NameI (nebrTabFac),
  NameR (rNebrShell),
  NameI (sizeHistBondAng),
  NameI (sizeHistDihedAng),
  NameI (stepAvg),
...
void SingleStep ()
{
  ...
  if (stepCount >= stepEquil && (stepCount - stepEquil) %
     stepChainProps == 0) {
    AccumDihedAngDistn (1);
    AccumBondAngDistn (1);
  }
  if (stepCount % stepAvg == 0) {
    AccumProps (2);
    AccumDihedAngDistn (2);
    AccumBondAngDistn (2);
    PrintSummary (stdout);
    AccumProps (0);
    AccumDihedAngDistn (0);
    AccumBondAngDistn (0);
  }
}
...
void AllocArrays ()
{
  ...
  AllocMem (histDihedAng, sizeHistDihedAng, real);
  AllocMem (histBondAng, sizeHistBondAng, real);
}
...
void PrintSummary (FILE *fp)
{
  ...
  fprintf (fp,
     ...
     PropEst (kinEnergy));
  fprintf (fp, "\n");
  fprintf (fp, "constraint devs: %.3e cycles: %d %d\n",
     consDevL, nCycleR, nCycleV);
  fprintf (fp, "dihed ang\n");
  for (n = 0; n < sizeHistDihedAng; n ++) {
    hVal = (n + 0.5) * 360. / sizeHistDihedAng;
    fprintf (fp, "%5.1f %.4f\n", hVal, histDihedAng[n]);
  }
  fprintf (fp, "bond ang\n");
  for (n = 0; n < sizeHistBondAng; n ++) {
    hVal = (n + 0.5) * 180. / sizeHistBondAng;
    fprintf (fp, "%5.1f %.4f\n", hVal, histBondAng[n]);
  }
  fflush (fp);
}
...
void ComputeChainAngleForces ()
{
  VecR dr1, dr2, w1, w2;
  real c, c11, c12, c22, cCon, cd, f, aCon = 868.6;
  int i, n, nn;

  cCon = cos (M_PI - bondAng);
  for (n = 0; n < nChain; n ++) {
    for (i = 0; i < chainLen - 2; i ++) {
      ...
      c = c12 / cd;
      f = - aCon * (c - cCon);
      VSSAdd (w1, c12 / c11, dr1, -1., dr2);
      VScale (w1, f / cd);
      VSSAdd (w2, 1., dr1, - c12 / c22, dr2);
      VScale (w2, f / cd);
      VVAdd (mol[nn].ra, w1);
      VVSub (mol[nn + 1].ra, w1);
      VVSub (mol[nn + 1].ra, w2);
      VVAdd (mol[nn + 2].ra, w2);
...
void AccumDihedAngDistn (int icode)
{
  VecR dr1, dr2, dr3, w;
  ...
  if (icode == 0) {
    for (j = 0; j < sizeHistDihedAng; j ++) histDihedAng[j] = 0.;
  } else if (icode == 1) {
    for (n = 0; n < nChain; n ++) {
      for (i = 0; i < chainLen - 3; i ++) {
        nn = n * chainLen + i;
        ...
        j = dihedAng * sizeHistDihedAng / (2. * M_PI);
        ++ histDihedAng[j];
      }
    }
  } else if (icode == 2) {
    t = 0.;
    for (j = 0; j < sizeHistDihedAng; j ++) t += histDihedAng[j];
    for (j = 0; j < sizeHistDihedAng; j ++) histDihedAng[j] /= t;
  }
}

void AccumBondAngDistn (int icode)
{
  VecR dr1, dr2;
  real bondAng, c11, c12, c22, t;
  int i, n, j, nn;

  if (icode == 0) {
    for (j = 0; j < sizeHistBondAng; j ++) histBondAng[j] = 0.;
  } else if (icode == 1) {
    for (n = 0; n < nChain; n ++) {
      for (i = 0; i < chainLen - 2; i ++) {
        nn = n * chainLen + i;
        ...
        c12 = VDot (dr1, dr2);
        c22 = VLenSq (dr2);
        bondAng = M_PI - acos (c12 / sqrt (c11 * c22));
        j = bondAng * sizeHistBondAng / M_PI;
        ++ histBondAng[j];
      }
    }
  } else if (icode == 2) {
    t = 0.;
    for (j = 0; j < sizeHistBondAng; j ++) t += histBondAng[j];
    for (j = 0; j < sizeHistBondAng; j ++) histBondAng[j] /= t;
  }
}
