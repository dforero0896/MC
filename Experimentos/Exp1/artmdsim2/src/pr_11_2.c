...
int moreCycles, randSeed, runId, stepAvg, stepCount, stepLimit;
real bondAng, bondLen, helixOrder, kinEnVal, tempFinal, tempInit,
   tempReduceFac, totEnVal, twistAng, uCon;
int *cellList, chainLen, helixPeriod, nDof, nSite, stepAdjustTemp, stepReduceTemp;
int stepSnap;
...
void SingleStep ()
{
  ...
  EvalProps ();
  EvalHelixOrder ();
  if (stepCount % stepReduceTemp == 0 && temperature > tempFinal) {
    temperature *= tempReduceFac;
    AdjustTemp ();
  }
  if (stepCount % stepAdjustTemp == 0 || stepCount < stepAdjustTemp &&
...
void EvalHelixOrder ()
{
  VecR dr1, dr2, rc, rcSum;
  real f;
  int k;

  VZero (rcSum);
  for (k = 0; k < P.nLink; k ++) {
    VSub (dr1, site[k + 1].r, site[k].r);
    VSub (dr2, site[k + 2].r, site[k + 1].r);
    VCross (rc, dr1, dr2);
    f = VLen (rc);
    if (f > 0.) VVSAdd (rcSum, 1. / f, rc);
  }
  helixOrder = VLen (rcSum) / P.nLink;
}

void PrintSummary (FILE *fp)
{
  fprintf (fp,
     "%5d %8.4f %7.4f %7.4f %7.4f",
     stepCount, timeNow, totEnVal, kinEnVal, helixOrder);
  fprintf (fp, "\n");
  fflush (fp);
}
...
void PutConfig ()
{
  ...
  fOk = 1;
  blockSize = (NDIM + 4) * sizeof (real) + 3 * sizeof (int) +
     nSite * NDIM * sizeof (short);
  if ((fp = fopen (fileName[FL_SNAP], "a")) != 0) {
    ...
    WriteF (kinEnVal);
    WriteF (helixOrder);
    AllocMem (rI, NDIM * nSite, short);
    for (n = 0; n < nSite; n ++) {
      VDiv (w, site[n].r, region);
      VAddCon (w, w, 0.5);
      VScale (w, SCALE_FAC);
      VToLin (rI, NDIM * n, w);
    }
    WriteFN (rI, NDIM * nSite);
    free (rI);
    if (ferror (fp)) fOk = 0;
    fclose (fp);
  } else fOk = 0;
